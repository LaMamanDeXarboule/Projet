...
private:
	...
	public:
			...
			const sf::Vector2f& position, const sf::Vector2f& tileCount, std::size_t rowSize, TextureResource& tr)
		{
			...
			sf::Vector2f layerOffset(float(offset.x), float(offset.y)); //conversion en float
				...
				for (auto y = yPos; y < yPos + tileCount.y; ++y)
				{
					for (auto x = xPos; x < xPos + tileCount.x; ++x)
					{
							...
							&& tileIDs[idx].ID < (ts->getFirstGID() + ts->getTileCount()))
						{
							...
							sf::Vector2f tileOffset(float(x * tileSize.x), float(y * tileSize.y)); //conversion en float
							...
							Tile tile =
							{
								...
								sf::Vertex(tileOffset + sf::Vector2f(float(tileSize.x), 0.f), vertColour, tileIndex + sf::Vector2f(float(tileSize.x), 0.f)), //conversion en float
								sf::Vertex(tileOffset + sf::Vector2f(float(tileSize.x), float(tileSize.y)), vertColour, tileIndex + sf::Vector2f(float(tileSize.x), float(tileSize.y))), //conversion en float
								sf::Vertex(tileOffset + sf::Vector2f(0.f, float(tileSize.y)), vertColour, tileIndex + sf::Vector2f(0.f, float(tileSize.y))) //conversion en float
#endif
#ifdef __ANDROID__								
								sf::Vertex(tileOffset, vertColour, tileIndex),
